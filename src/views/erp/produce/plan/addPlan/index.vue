...
<!--   -->
<!--      </router-link>-->
      <el-button type="primary"     icon="Back" @click="returnPlan"
      ...
      <div>
        ...
        <el-button type="primary" plain @click="submitPlanForm">保存</el-button>
      </div>
    </el-row>
...
function getPlanTotalCost(){
  if (form.value.productList.length > 0) {
    let totalCount = 0;
    let totalCost = 0;
    form.value.productList.forEach((row) => {
      totalCount++;
      row.usageAmount = row.singleGroupNumber * form.value.groupNumber
      totalCost += row.usageAmount * row.costPrice;
      row.amountPrice = row.usageAmount * row.costPrice;
...
function getBomList(){
  queryBom.value.auditId = "1"
  queryBom.value.status = "0"
